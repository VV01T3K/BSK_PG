...
async function startSession(page: Page) {
  ...
  await expect(button(page, "Exchange data")).toBeEnabled();
}
...
test("gates each step until its prerequisites are met", async ({ page }) => {
  ...
  await expect(button(page, "Exchange data")).toBeDisabled();
  await expect(button(page, "Run test")).toBeDisabled();
  await expect(button(page, "Close session")).toBeDisabled();
});

test("completes the register -> authenticate -> exchange happy path", async ({ page }) => {
  await register(page);
  ...
  await button(page, "Exchange data").click();
  await expect(stateValue(page, "Service")).toHaveText("encrypted exchange complete");
